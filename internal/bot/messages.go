...
func privacyText(lang language, privacyURL string, voiceExternal bool) string {
	...
	switch lang {
	case langKK:
		return "Құпиялылық: жіберілген бастапқы контент кезекте шифрланған түрде қысқа уақыт сақталады және логтарға жазылмайды. Belcanto аудиті қосылса, қауіпсіз AI финалистері мен бағалары, сондай-ақ медиа өзгерісінің әрекеті мен көзі логталуы мүмкін; full режимінде Pexels asset ID және анонимді іздеу сұрауы да жазылады. Жасалған жауаптар, Belcanto Threads нобайлары және ашық feedback әдетте 7 күн сақталады. Threads нобайының дәл мәтіні Meta-ға тек оператор растағаннан кейін жіберіледі." + voiceKK + " /delete_me барлық профиль деректерін жояды.\n\n" + policyLink("Толық саясат", privacyURL)
	case langEN:
		return "Privacy: submitted source content is held briefly as an encrypted queue payload and is never written to logs. When Belcanto audit logging is enabled, safe AI-generated finalists and scores plus media-change actions and sources may be logged; full mode also records the Pexels asset ID and anonymous search query. Generated replies, Belcanto Threads drafts, and explicit feedback are normally retained for 7 days. The exact Threads preview is sent to Meta only after an operator confirms publication." + voiceEN + " /delete_me removes all profile data.\n\n" + policyLink("Full policy", privacyURL)
	default:
		return "Приватность: отправленный исходный контент кратковременно хранится в очереди только в зашифрованном виде и не попадает в логи. Если включён аудит Belcanto, логироваться могут безопасные AI-финалисты и оценки, а также действия и источник при смене медиа; в full-режиме — ещё Pexels asset ID и анонимный поисковый запрос. Сгенерированные ответы, черновики Belcanto Threads и явный feedback обычно хранятся 7 дней. Точный текст Threads-поста передаётся Meta только после подтверждения оператора." + voiceRU + " /delete_me удаляет все данные профиля.\n\n" + policyLink("Полная политика", privacyURL)
	}
}
