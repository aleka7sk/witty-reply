...
7. Set `BELCANTO_OPERATOR_IDS` to your numeric Telegram ID and `THREADS_PROVIDER=fake`. Send `/threads`, choose a goal, then test a short real text material. For `trial`, confirmed material is mandatory; the bot must reject the no-material action before generation. Test `✨ Без материала дня` with `reach`, `replies`, `trust`, or `community`. Confirm that Telegram shows exactly five ranked finalists, labels each one as material-backed or evergreen, and exposes no draft, media, refinement, or publication controls before one is selected.
8. Restart once while the five-finalist choice is open and confirm that the same ranked options return without another generation. Choose one option and verify that only its exact text becomes the WYSIWYG draft and recent-text protection input; the other four must not affect the next generation. An evergreen override from a material-backed portfolio must say that the material was not used. Re-deliver the same selection update immediately and confirm that it returns the same draft. Then refine or publish that draft and replay the old selection again: it must return only a stale notice, with no obsolete draft controls. Another or stale selection cannot replace the accepted choice.
9. Verify the exact text-only preview and fake publication. Confirm that photo controls appear only after finalist selection. Then create another workflow, attach/replace/remove one own photo, inspect every exact photo-plus-caption preview, and press its rights/publication button twice. Only the first confirmation may publish. To test licensed media too, first set `THREADS_PHOTO_PROVIDER=pexels` and a real `PEXELS_API_KEY`, restart the app, then use `Подобрать фото в Pexels` and `Другое фото из Pexels`; otherwise skip those controls. A manual Pexels change appears as `belcanto_threads_media_changed`; it does not rewrite the immutable editorial-review event.
10. For a real editorial-pipeline test without a Meta publication risk, keep `THREADS_PROVIDER=fake`, switch to `AI_PROVIDER=anthropic`, add an Anthropic API key, and restart. The Threads planner, writer, and reviewer use `THREADS_AI_MAX_TOKENS=8192`, `THREADS_AI_CALL_TIMEOUT=90s`, and a shared end-to-end `THREADS_AI_TIMEOUT=300s` by default. A new portfolio uses those three stages once; displaying it again or selecting a stored finalist must not make another Anthropic request.
...
Application migrations are additive and idempotent in v2, so a rollback to another image that already understands schema v5 does not require reversing the schema. Do **not** start a pre-v5 image after migration 005: it does not understand the durable `candidates_ready` state, and its retention job does not account for the finalist-set foreign keys. Draining only the currently open selection screens is insufficient because selected historical sets remain linked to drafts and media.

Prefer a roll-forward repair. An emergency downgrade below v5 requires a database backup plus an explicit, separately reviewed data migration that removes or transforms every v5 finalist aggregate before the old application starts; this runbook does not authorize an ad-hoc deletion of those audit rows. After a compatible rollback or roll-forward, validate readiness, `/start`, and one fake-provider generation before restoring normal traffic.
