...
6. A signed, owner-scoped selection chooses exactly one finalist. The editor's first-ranked option is a recommendation: with material it is material-backed, while the operator may explicitly override it with another fact-safe evergreen option. Telegram labels that provenance on every option and derives the selected preview label from the durable finalist set, not merely from the existence of brief material. The accepted choice atomically becomes the exact WYSIWYG current draft and is the only finalist added to recent-text protection. The other four remain outside similarity checks. Replaying the accepted selection returns the same draft without another Anthropic request or duplicate draft only while that draft is still actionable; after refinement or publication, the retry receives a stale notice without obsolete controls. Callbacks for an obsolete selection revision are rejected.
