...
- A Belcanto `ThreadBrief` durably stores the operator ID, fixed objective, workflow state, material kind, optional raw text material, replay-protection update IDs, and the resulting draft link. This allows goal/material collection to survive a retry or process restart. It is owner-scoped and normally removed by the same default seven-day cleanup, or immediately with the operator's profile through `/delete_me`.
- Belcanto Threads drafts retain the delivered winner as the preview, plus their owned brief link, objective, selected scenario, voice, provider/model identifiers, revision, selected format, an application-derived fixed fallback photo query, publication state, Meta container/post identifiers, and permalink when returned. Evidence remains in the owner-scoped brief and content-free evidence digests in the audit. An attached or licensed photo is resized, re-encoded as JPEG to remove EXIF/GPS metadata, stored with an owner check and source provenance, and normally removed by the same default seven-day cleanup. The Meta access token, Pexels API key, and Telegram file URL are never stored in a brief, draft, or media row.
- The Belcanto editorial audit is a separate structured operational log event. Its default `full` mode contains bounded objective/scenario metadata, numeric scores, winner, and an opaque generation ID; for no-material evergreen generations it also contains locally safe finalist texts and concise reviewer notes. Material-backed generations automatically omit every model-authored free-form field and retain secret-keyed, domain-separated HMAC-SHA-256 digests, scores, and bounded IDs only. Rejected output is always represented only by a safe code and keyed digest. A separate media-change audit records the draft revision, action, source and, in `full` mode only, the Pexels asset ID; it never logs a search query. `metadata` mode omits finalist text, free-form notes, and asset IDs; `off` emits neither event. Neither event contains the raw operator material, a raw Telegram ID, prompt, recent-post list, photo bytes, access token, or hidden model reasoning. Log-backend retention is deployment-controlled and should be limited to no more than thirty days.
...
- `/threads` lets an allowlisted Belcanto operator choose a goal, submit or explicitly skip material, inspect the resulting preview, replace the text, attach/replace/remove a photo, or cancel the workflow; only the explicit publish button sends the approved preview to Meta.
